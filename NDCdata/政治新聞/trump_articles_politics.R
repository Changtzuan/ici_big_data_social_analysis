...
setwd("D:/CJ/TRUMPdata/政治新聞")
# 載入CSV
df <- read_csv("政治新聞_2025-04-30.csv")
...
# 建立資料夾
dir.create("trump_articles_politics", showWarnings = FALSE)

# 儲存符合的全文為 .txt
walk2(df_trump$識別碼, df_trump$新聞全文, ~ {
  write_lines(.y, file = paste0("trump_articles_politics/", .x, ".txt"))
})

# Using readr
write_csv(
  df_trump %>% select(-新聞全文),
  file = "trump_articles_politics.csv",
  append = FALSE
)