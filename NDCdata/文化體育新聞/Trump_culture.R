...
setwd("~/Documents/NCCU/大三下/大數據/final project/文化體育新聞")
# 載入CSV
df <- read_csv("文化體育新聞_2025-04-29.csv")
...
# 建立資料夾
dir.create("trump_articles_culture", showWarnings = FALSE)

# 儲存符合的全文為 .txt
walk2(df_trump$識別碼, df_trump$新聞全文, ~ {
  write_lines(.y, file = paste0("trump_articles_culture/", .x, ".txt"))
})

# Using readr
write_csv(
  df_trump %>% select(-新聞全文),
  file = "trump_articles_culture.csv",
  append = FALSE
)
